...
public class PTVPlayerActivity extends Activity {
	...
	public static Intent startActivity(Activity activity, Media media) {
		return startActivity(activity, media, true, 0);
	}

	public static Intent startActivity(Activity activity, Media media, boolean startImmediately) {
		return startActivity(activity, media, startImmediately, 0);
	}

	public static Intent startActivity(Activity activity, Media media, boolean startImmediately, int startPosition) {
		Intent i = new Intent(activity, PTVPlayerActivity.class);
		i.putExtra(EXTRA_MEDIA, media);
		i.putExtra(EXTRA_START_IMMEDIATELY, startImmediately);
		i.putExtra(EXTRA_START_POSITION, startPosition);
	...
	private void startVideoPlayer() {
		...
		mSelectedMedia = (Media) getIntent().getSerializableExtra(EXTRA_MEDIA);
		boolean mShouldStartPlayback = getIntent().getBooleanExtra(EXTRA_START_IMMEDIATELY, true);
