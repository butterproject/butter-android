...
public class XMLRPCClient implements XMLRPCClientInterface {
    ...
    public XMLRPCClient(URI uri, String httpuser, String httppasswd) {
        this(uri, httpuser, httppasswd, "XMLRPC-Client");
    }

    public XMLRPCClient(URI uri, String httpuser, String httppasswd, String useragent) {
        mPostMethod = new HttpPost(uri);
        mPostMethod.addHeader("Content-Type", "text/xml");
        mPostMethod.addHeader("charset", "UTF-8");
        mPostMethod.addHeader("User-Agent", useragent);
            ...
            try {
                ...
                try {
                    ...
                    if (TextUtils.isEmpty(responseString)) {
                        ...
                        Log.e("XMLRPC", "HTTP error document received from the server: " + responseString);
                    }
                ...
                switch (e.getFaultCode()) {
                    case 403:
                        broadcastAction(BROADCAST_ACTION_XMLRPC_INVALID_CREDENTIALS);
                        break;
                    case 425:
                        broadcastAction(BROADCAST_ACTION_XMLRPC_TWO_FA_AUTH);
                        break;
                ...
                if (mIsWpcom) {
                    ...
                    broadcastAction(BROADCAST_ACTION_XMLRPC_INVALID_SSL_CERTIFICATE);
                }
                throw e;
