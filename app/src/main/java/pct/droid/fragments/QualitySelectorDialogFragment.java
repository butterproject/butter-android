...
import pct.droid.R;
import pct.droid.adapters.QualityAdapter;
...
public class QualitySelectorDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if(getArguments().containsKey(QUALITIES) && getActivity() instanceof Listener) {
            ...
            QualityAdapter adapter = new QualityAdapter(getActivity(), getArguments().getStringArray(QUALITIES));
            listView.setAdapter(adapter);
            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                    QualityAdapter adapter = (QualityAdapter) adapterView.getAdapter();
